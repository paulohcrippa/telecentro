							...
							<form class="form-horizontal" role="form"  method = "POST" action = "processa/proc_cont_atendimento.php">
								...
								<div class="step33">
									...
									<div class="col-sm-3">
										...
										<select class="form-control" name="pessoa" readonly ><option>Selecionar Pessoa</option>
											<?php 
												$query = mysql_query ("select * FROM tb_pessoa ORDER BY NMPESSOA ASC");
												while($res=mysql_fetch_array($query)){

													echo "<option value='{$res["CDPESSOA"]}' ".(($atendimento['CDPESSOA'] == $res["CDPESSOA"])? 'selected' : '').">{$res["NMPESSOA"]}</option>";
												}
											?>		
										</select>										
									</div>
